...
20\% retention threshold, restricted to the 17 descriptors selected by two or
more methods ($p = 81$, $n = 15{,}542$). Rows are ordered by decreasing
agreement level (right axis). The complete 36-feature matrix, including the 19
...
directly evidenced by the 11 $\xi_n$-exclusive descriptors in
...
We presented a comparative benchmark of Chatterjee's $\xi_n$ against established dependence measures (DC, MI, MIC) across 48 feature-selection configurations and 30 repeated resampling runs on a superconductivity dataset ($n = 15{,}542$, $p = 81$).
...
all 36 descriptors appearing in at least one selected subset at the 20\%
retention threshold. The 19 single-method-only descriptors (agreement level~1)
are retained here for completeness but excluded from the main-text
Figure~\ref{fig:agreement}, which focuses on the 17 analytically informative
consensus features (agreement $\geq 2$).
 
\begin{figure}[htb]
\centering
\includegraphics[width=\columnwidth]{full_feature_agreement_matrix}
\caption{Complete feature selection agreement matrix for all 36 descriptors
