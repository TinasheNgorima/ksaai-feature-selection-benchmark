...
def score_xi(X_df: pd.DataFrame, y: np.ndarray) -> pd.DataFrame:
    ...
    for col in tqdm(X_df.columns, desc="ξₙ", leave=False):
        try:
            ...
            scores[col] = 0.0
    return pd.DataFrame(list(scores.items()), columns=["Feature", r"$\xi_n$ Score"])
...
def plot_agreement_matrix(
    ...
    xi_feat  = set(df_xi.sort_values(r"$\xi_n$ Score",  ascending=False).head(n_top)["Feature"])
    ...
    pres = pd.DataFrame({
        ...
        r"$\xi_n$"     : [1 if f in xi_feat  else 0 for f in all_features],
    })
    pres["Agreement"] = pres[["DC", "MI", "MIC", r"$\xi_n$"]].sum(axis=1)
    ...
    matrix = pres[["DC", "MI", "MIC", r"$\xi_n$"]].values
