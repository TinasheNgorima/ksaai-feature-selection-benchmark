...
METHOD_DISPLAY = {"DC": "DC", "MI": "MI", "MIC": "MIC", "XI": r"$\xi_n$"}
...
def run_config(
    ...
    cv_rmse = float(-cross_val_score(best_pipe, X_tr_sel, y_train, cv=KFold(n_splits=N_CV_FOLDS, shuffle=True, random_state=SEED_MODEL), scoring="neg_root_mean_squared_error").mean())
...
def compute_and_save_feature_scores(X_train: np.ndarray, y_train: np.ndarray, feature_names: list):
    ...
    score_cols = {"DC": "DC Score", "MI": "MI Score", "MIC": "MIC Score", "XI": r"$\xi_n$ Score"}
...
def main():
    ...
    for method, threshold, model_name in tqdm(configs, desc="Configs"):
        try:
            ...
            rows.append(row)
            log.info(
                f"  {METHOD_DISPLAY[method]:4s} {threshold:2d}% {model_name:12s} "
    ...
    import json
    pivot_r2  = results_df.pivot_table(index=["model","threshold_pct"], columns="method", values="test_r2")
    pivot_mae = results_df.pivot_table(index=["model","threshold_pct"], columns="method", values="test_mae")
    stat_r2,  p_r2  = friedmanchisquare(*[pivot_r2[m].values  for m in METHODS])
    stat_mae, p_mae = friedmanchisquare(*[pivot_mae[m].values for m in METHODS])
    log.info(f"Friedman R2:  chi2={stat_r2:.3f}  p={p_r2:.4f}")
    log.info(f"Friedman MAE: chi2={stat_mae:.3f}  p={p_mae:.4f}")
    with open(RESULTS_DIR / "friedman_results.json", "w") as fh:
        json.dump({"r2": {"statistic": stat_r2, "p_value": p_r2}, "mae": {"statistic": stat_mae, "p_value": p_mae}}, fh, indent=2)
    nemenyi_r2  = sp.posthoc_nemenyi_friedman(pivot_r2.values)
    nemenyi_mae = sp.posthoc_nemenyi_friedman(pivot_mae.values)
    nemenyi_r2.columns  = METHODS; nemenyi_r2.index  = METHODS
    nemenyi_mae.columns = METHODS; nemenyi_mae.index = METHODS
    nemenyi_r2.to_csv(RESULTS_DIR  / "nemenyi_matrix_r2.csv")
    nemenyi_mae.to_csv(RESULTS_DIR / "nemenyi_matrix_mae.csv")
    log.info("Friedman and Nemenyi results saved.")
